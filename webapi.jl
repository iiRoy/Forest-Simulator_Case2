...
route("/simulations", method = POST) do
    ...
    model = forest_fire(griddims=(x,y), probability_of_spread=(probability_of_spread), density =(density), south_wind_speed=(south_wind_speed), west_wind_speed=(west_wind_speed))
    id = string(uuid1())
    instances[id] = model
